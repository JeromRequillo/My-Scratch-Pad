...
;@Ahk2Exe-SetDescription Shortcut Key - Workflow Automation Suite
...
global active_hotkey_profile := "Default"
global mainGui        := ""
global hotkeyList     := []
...
LoadSettings() {
    ...
    global folderLauncherList, active_hotkey_profile
    ...
    ; --- Custom text hotkeys (per Hotkey Profile) ---
    active_hotkey_profile := IniRead("settings.ini", "Settings", "ActiveHotkeyProfile", "Default")
    hotkeyList := LoadHotkeyProfileData(active_hotkey_profile)
...
DoInvoiceHotkey() {
    ...
    ; Suspend ALL hotkeys for the duration of the send so the typed
    ; characters can never re-trigger this or any other hotkey — while
    ; still using real SendInput keystrokes for app compatibility.
    Suspend(true)
    try
...
DoVatHotkey() {
    ...
    Send("^c")
    if !ClipWait(3) {
        ; Retry once — the source app may still be busy/slow (e.g. waiting
        ; on its own server) and missed the first Ctrl+C.
        Send("^c")
        if !ClipWait(3) {
...
SendTextSafe(txt) {
    ; Suspend ALL hotkeys (system, custom text, folder launcher) for the
    ; duration of the send. This blocks any of them from re-triggering on
    ; the keystrokes we're about to type, while letting us use real
    ; SendInput keystrokes — which most apps (including JS-based web
    ; systems) expect and handle correctly, unlike SendText.
    Suspend(true)
    try
        SendInput(txt)
    finally
...
; --- Custom Text Hotkey Profiles (independent of Invoice profiles) ---

GetHotkeyProfileList() {
    profiles := ["Default"]
    try {
        allSections := IniRead("settings.ini")
        Loop Parse, allSections, "`n", "`r" {
            if (SubStr(A_LoopField, 1, 8) == "Hotkeys_") {
                pName := SubStr(A_LoopField, 9)
                found := false
                for p in profiles
                    if (p == pName)
...
HotkeysSectionName(pName) {
    return (pName == "Default") ? "Hotkeys" : "Hotkeys_" . pName
}

HotkeyStateSectionName(pName) {
    return (pName == "Default") ? "HotkeyState" : "HotkeyState_" . pName
}

LoadHotkeyProfileData(pName) {
    list := []
    hkSection := HotkeysSectionName(pName)
    stSection := HotkeyStateSectionName(pName)
    try {
        hkSections := IniRead("settings.ini", hkSection)
        Loop Parse, hkSections, "`n", "`r" {
            ...
            if (pos > 0) {
                ...
                enabledVal := IniRead("settings.ini", stSection, hkKey, "1")
                list.Push({key: hkKey, txt: hkTxt, enabled: (enabledVal == "1")})
            }
        }
    } catch {
        if (pName == "Default")
            list := [{key: "!S", txt: "SAMPLE TXT", enabled: true}]
    }
    return list
}
...
LaunchGUI() {
    ...
    mainGui := Gui("-MaximizeBox", "🎯 Shortcut Key v1.0")
...
The current sequence number is automatically saved to settings.ini after every hotkey press. Progress is never lost, even if the application is closed unexpectedly.
    ...
    mainGui.SetFont("bold s13", "Segoe UI")
    mainGui.Add("Text", "x20 y48 w280 h28 c0x0055AA", "⌨ Custom Text Hotkeys")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x315 y52 w55 h22 +0x200", "Profile:")
    hotkeyProfileList := GetHotkeyProfileList()
    ddHotkeyProfile := mainGui.Add("DropDownList", "x368 y50 w130 h150", hotkeyProfileList)
    for i, p in hotkeyProfileList {
        if (p == active_hotkey_profile) {
            ddHotkeyProfile.Value := i
            break
        }
    }
    btnNewHotkeyProfile := mainGui.Add("Button", "x502 y50 w35 h24", "➕")
    btnDelHotkeyProfile := mainGui.Add("Button", "x540 y50 w35 h24", "🗑")
    mainGui.SetFont("s9 Norm cGray", "Segoe UI")
    mainGui.Add("Text", "x20 y76 w630 h18",
    ...
    ddHotkeyProfile.OnEvent("Change", SwitchHotkeyProfile)
    btnNewHotkeyProfile.OnEvent("Click", NewHotkeyProfile)
    btnDelHotkeyProfile.OnEvent("Click", DeleteHotkeyProfile)
    ...
    mainGui.Add("Text", "x30 y68 w380 h30", "🎯 Shortcut Key  v1.0")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x30 y100 w85 h20 +0x200", "Version:")
    mainGui.Add("Text", "x118 y100 w270 h20", "1.0.0 ")
...
v4.1  —  Initial public release. Features included auto-invoice number generation with configurable prefix, suffix, and digit length, a VAT deductor hotkey, and system tray integration.
    ...
    SwitchHotkeyProfile(*) {
        global active_hotkey_profile, hotkeyList
        active_hotkey_profile := ddHotkeyProfile.Text
        IniWrite(active_hotkey_profile, "settings.ini", "Settings", "ActiveHotkeyProfile")
        hotkeyList := LoadHotkeyProfileData(active_hotkey_profile)
        RegisterCustomHotkeys()
        editTxt.Value := ""
        ddModifier.Value := 1
    ...
    NewHotkeyProfile(*) {
        global active_hotkey_profile, hotkeyList
        newName := InputBox("Enter a name for the new hotkey profile:", "Create New Hotkey Profile", "w320 h120").Value
        if (newName == "" || newName == 0)
            return
        pList := GetHotkeyProfileList()
        for p in pList {
            if (p == newName) {
                MsgBox("A hotkey profile named '" . newName . "' already exists. Please choose a different name.", "Duplicate Name", 48)
                return
            }
        }
        ; Create the section on disk (write then remove a marker key, leaving an
        ; empty section header so the profile persists and shows up in the list)
        IniWrite("1", "settings.ini", HotkeysSectionName(newName), "_init")
        try IniDelete("settings.ini", HotkeysSectionName(newName), "_init")

        newList := GetHotkeyProfileList()
        ddHotkeyProfile.Delete()
        for p in newList
            ddHotkeyProfile.Add([p])
        for i, p in newList {
            if (p == newName) {
                ddHotkeyProfile.Value := i
                break
    ...
    DeleteHotkeyProfile(*) {
        global active_hotkey_profile, hotkeyList
        if (ddHotkeyProfile.Text == "Default") {
            MsgBox("The Default hotkey profile cannot be deleted.", "Action Not Allowed", 48)
            return
        }
        pName := ddHotkeyProfile.Text
        confirm := MsgBox("Are you sure you want to permanently delete the hotkey profile '" . pName . "'? All hotkeys stored in this profile will be lost. This action cannot be undone.",
            "Confirm Deletion", "YesNo 48")
        if (confirm != "Yes")
            return
        try IniDelete("settings.ini", HotkeysSectionName(pName))
        try IniDelete("settings.ini", HotkeyStateSectionName(pName))
        ...
        for p in newList
            ddHotkeyProfile.Add([p])
        ddHotkeyProfile.Value := 1
        hotkeyList := LoadHotkeyProfileData("Default")
        RegisterCustomHotkeys()
        editTxt.Value := ""
        ddModifier.Value := 1
        ddKey.Value := 1
    ...
    CheckConflict(newKey, excludeKey := "") {
        ...
        ; Check against the FULL hotkey list, not just whatever rows are
        ; currently visible in the LV (a search filter can hide rows).
        for hk in hotkeyList {
            if (hk.key == excludeKey)
                continue
            if (hk.key == newKey)
                return "'" . newKey . "' is already in use in the hotkey list. Each hotkey must be unique."
        }
    ...
    ; Finds the index of a hotkey entry in hotkeyList by its key string.
    ; Returns 0 if not found. Using the key (not a row number) means this
    ; stays correct even while the list is filtered by search.
    FindHotkeyByKey(key) {
        global hotkeyList
    ...
    AddUpdateHotkey(*) {
        ...
        conflictMsg := CheckConflict(newKey, newKey)
        if (conflictMsg != "") {
        ...
        existingIdx := FindHotkeyByKey(newKey)
        if (existingIdx > 0) {
            hotkeyList[existingIdx].txt := storedTxt
        } else {
            hotkeyList.Push({key: newKey, txt: storedTxt, enabled: true})
        }
        editTxt.Value := ""
        ddModifier.Value := 1
        ddKey.Value := 1
        RefreshListView(editSearch.Value)
    }
    ...
    DeleteHotkey(*) {
        ...
        keyToDelete := LV.GetText(selectedRow, 2)
        idx := FindHotkeyByKey(keyToDelete)
        if (idx > 0)
            hotkeyList.RemoveAt(idx)
        editTxt.Value := ""
        ddModifier.Value := 1
        ddKey.Value := 1
        RefreshListView(editSearch.Value)
    ...
    ToggleHotkey(*) {
        ...
        idx := FindHotkeyByKey(rawKey)
        if (idx == 0)
            return
        hotkeyList[idx].enabled := !hotkeyList[idx].enabled
        if (hotkeyList[idx].enabled) {
            try {
                realTxt   := StrReplace(hotkeyList[idx].txt, "\n", "`n")
                boundFunc := CreateHotkeyFunc(realTxt)
    ...
    MoveRowUp(*) {
        ...
        if (idx <= 1)
            return
        temp := hotkeyList[idx - 1]
        hotkeyList[idx - 1] := hotkeyList[idx]
        hotkeyList[idx] := temp
        RefreshListView(editSearch.Value)
        ReselectByKey(rawKey)
    }
    ...
    MoveRowDown(*) {
        ...
        rawKey := LV.GetText(selectedRow, 2)
        idx := FindHotkeyByKey(rawKey)
        if (idx == 0 || idx >= hotkeyList.Length)
            return
        temp := hotkeyList[idx + 1]
        hotkeyList[idx + 1] := hotkeyList[idx]
        hotkeyList[idx] := temp
    ...
    SaveSettings(*) {
        ...
        for hk in hotkeyList {
            ...
            for si, sk in sysKeys {
                if (lvKey == sk && sk != managerKey) {
                    MsgBox("Hotkey Conflict Detected`n`nThe " . sysLabels[si] . " hotkey '" . sk . "' is already assigned to a Custom Text Hotkey ('" . lvKey . "').`n`nPlease resolve this conflict before saving.", "Hotkey Conflict", 48)
                    return
                }
            }
        }
        ...
        Loop FL_LV.GetCount() {
            ...
            for hk in hotkeyList {
                if (hk.key == flHK) {
                    MsgBox("Hotkey Conflict Detected`n`nA Folder Launcher entry uses hotkey '" . flHK . "' which is already assigned to Custom Text Hotkey '" . hk.key . "'. Please resolve this before saving.", "Hotkey Conflict", 48)
                    return
                }
            }
        }
        ...
        hkSection := HotkeysSectionName(active_hotkey_profile)
        stSection := HotkeyStateSectionName(active_hotkey_profile)
        try IniDelete("settings.ini", hkSection)
        try IniDelete("settings.ini", stSection)
        ; hotkeyList is now kept live-accurate by Add/Update/Delete/Toggle/Move,
        ; so write it directly — this also saves correctly even if a search
        ; filter is currently active in the list above (filtered rows are not lost).
        for hk in hotkeyList {
            IniWrite(hk.txt, "settings.ini", hkSection, hk.key)
            IniWrite(hk.enabled ? "1" : "0", "settings.ini", stSection, hk.key)
        }
        ...
        MsgBox("All settings have been saved successfully.`n`n"
            ...
            . "Hotkey Profile : " . active_hotkey_profile . "`n"
...
LaunchVolumeControl() {
    ...
    btnMute.OnEvent("Click", FlashMute)

    FlashMute(*) {
        SoundSetMute(-1)
        volLabel.SetFont("c0xCC0000")
        ToolTip("Mute toggled")
        SetTimer(() => ToolTip(), -1200)
        SetTimer(() => volLabel.SetFont("cDefault"), -400)
    }
    ...
    glideState := {timer: "", target: 0, cur: 0}
    SetVol(target) {
        ; Animate the slider/label gliding toward the target value instead
        ; of snapping instantly, so preset clicks feel more dynamic.
        if (glideState.timer != "")
            SetTimer(glideState.timer, 0)
        glideState.target := target
        glideState.cur := volSlider.Value
        glideState.timer := GlideStep
        SetTimer(GlideStep, 15)

        GlideStep() {
            step := (glideState.target > glideState.cur) ? 3 : -3
            glideState.cur += step
            done := false
            if ((step > 0 && glideState.cur >= glideState.target) || (step < 0 && glideState.cur <= glideState.target)) {
                glideState.cur := glideState.target
                done := true
            }
            volSlider.Value := glideState.cur
            volLabel.Value  := glideState.cur . "%"
            SoundSetVolume(glideState.cur)
            if (done)
...
LaunchScreenDimmer() {
    ...
    ToggleDim(*) {
        targetAlpha := Round(dimSlider.Value * 2.55)
        if (fadeState.timer != "")
            SetTimer(fadeState.timer, 0)

        if (isDimmed) {
            ; Fade out, then destroy the overlay once fully transparent
            btnToggleDim.Text := "🌙 Enable"
            isDimmed := false
            fadeState.cur := targetAlpha
            fadeState.timer := FadeOutStep
            SetTimer(FadeOutStep, 15)
            ...
            FadeOutStep() {
                ...
                if (fadeState.cur <= 0) {
                    if (IsObject(overlayGui))
                        overlayGui.Destroy()
                    overlayGui := ""
                    SetTimer(FadeOutStep, 0)
                    return
                }
                if (IsObject(overlayGui))
                    WinSetTransparent(fadeState.cur, overlayGui)
            }
        } else {
            ; Create the overlay fully transparent, then fade it in to target strength
            overlayGui := Gui("+E0x80000 -Caption +ToolWindow +AlwaysOnTop")
            overlayGui.BackColor := GetOverlayColor()
            overlayGui.Show("x0 y0 w" . A_ScreenWidth . " h" . A_ScreenHeight . " NoActivate")
            WinSetTransparent(0, overlayGui)
            WinSetExStyle("+0x20", overlayGui)   ; click-through
            isDimmed := true
            btnToggleDim.Text := "☀ Disable"

            fadeState.cur := 0
            fadeState.timer := FadeInStep
            SetTimer(FadeInStep, 15)

            FadeInStep() {
                fadeState.cur += 18
                if (fadeState.cur >= targetAlpha) {
                    fadeState.cur := targetAlpha
                    SetTimer(FadeInStep, 0)
                }
                if (IsObject(overlayGui))
                    WinSetTransparent(fadeState.cur, overlayGui)
            }
...
LaunchDiskCleaner() {
    ...
    cleanGui.OnEvent("Close", (*) => (SetTimer(CleanStep, 0), cleanGui.Destroy()))
    ...
    ; --- Live status line (shows current action / current file) ---
    cleanGui.SetFont("s9 Norm c0x666666", "Segoe UI")
    statusTxt := cleanGui.Add("Text", "x10 y154 w360 h18", "Idle — ready to clean.")

    ; --- Animated progress bar ---
    progBar := cleanGui.Add("Progress", "x10 y176 w360 h18 cBlue Background0xE0E0E0", 0)

    ; --- Live counter (files removed so far / running tally) ---
    cleanGui.SetFont("bold s9 c0x005500", "Segoe UI")
    counterTxt := cleanGui.Add("Text", "x10 y198 w360 h18", "")

    resultTxt := cleanGui.Add("Edit", "x10 y220 w360 h70 +ReadOnly +Wrap +VScroll", "Results will appear here...")

    btnClean := cleanGui.Add("Button", "x10 y298 w170 h32", "🚀 Run Cleanup Now")
    btnClose := cleanGui.Add("Button", "x190 y298 w120 h32", "✖ Close")
    btnClose.OnEvent("Click", (*) => (SetTimer(CleanStep, 0), cleanGui.Destroy()))

    btnClean.OnEvent("Click", StartClean)

    ; ---- State used by the animated cleanup sequence ----
    ; Declared at this scope (not inside StartClean) so CleanStep is visible
    ; to the Close handlers above as well as to StartClean below.
    cleanQueue   := []   ; queue of work items, each processed one tick at a time
    tempFiles    := []   ; individual temp files queued for the "live delete" feed
    log          := ""
    deletedCount := 0
    running      := false
    totalSteps   := 0
    ...
    StartClean(*) {
        if (running)
            return
        if (!cbRecycle.Value && !cbTemp.Value && !cbPrefetch.Value) {
            resultTxt.Value := "Nothing selected to clean."
            return
        }
        ...
        btnClean.Text := "⏳ Cleaning..."
        resultTxt.Value := ""
        counterTxt.Text := ""
        progBar.Value := 0

        ; Build the queue of phases up front so the progress bar reflects real work
        if (cbRecycle.Value)
            cleanQueue.Push("recycle")
        if (cbTemp.Value) {
            ; Pre-scan temp files so we know the total count for an accurate bar
            tempPath := EnvGet("TEMP")
            Loop Files, tempPath . "\*.*", "FR" {
                tempFiles.Push(A_LoopFilePath)
            }
            cleanQueue.Push("temp")
        }
        if (cbPrefetch.Value)
            cleanQueue.Push("prefetch")

        totalSteps := cleanQueue.Length + tempFiles.Length
        progBar.Value := 0
        statusTxt.Text := "Starting cleanup..."

        SetTimer(CleanStep, 30)
    }

    CleanStep(*) {
        ; Advance one unit of work per tick so the UI stays responsive
        ; and the animation is actually visible to the user.
        if (currentPhase = "" && cleanQueue.Length) {
            currentPhase := cleanQueue.RemoveAt(1)
            if (currentPhase = "recycle")
                statusTxt.Text := "🗑 Emptying Recycle Bin..."
            else if (currentPhase = "temp")
                statusTxt.Text := "🧹 Scanning Temp folder..."
            else if (currentPhase = "prefetch")
                statusTxt.Text := "🧰 Clearing Prefetch (admin)..."
            return  ; let the status text repaint before doing the work
        }
        ...
        if (currentPhase = "recycle") {
            ...
            stepsDone++
            progBar.Value := Round((stepsDone / totalSteps) * 100)
            currentPhase := ""
            return
        }

        if (currentPhase = "temp") {
            if (tempIdx < tempFiles.Length) {
                tempIdx++
                f := tempFiles[tempIdx]
                try {
                    FileDelete(f)
                ...
                SplitPath(f, &fname)
                statusTxt.Text := "🧹 Deleting: " . fname
                counterTxt.Text := "Files removed: " . deletedCount . " / " . tempFiles.Length
                stepsDone++
                progBar.Value := Round((stepsDone / totalSteps) * 100)
                return
            } else {
                log .= "✅ Temp folder: " . deletedCount . " file(s) removed.`n"
                currentPhase := ""
                return
            }
        }

        if (currentPhase = "prefetch") {
            try {
        ...
        SetTimer(CleanStep, 0)
        progBar.Value := 100
        statusTxt.Text := "✅ Cleanup complete!"
        resultTxt.Value := log != "" ? log : "Nothing selected to clean."
        btnClean.Enabled := true
        btnClean.Text := "🚀 Run Cleanup Now"
        running := false
    }

    cleanGui.Show("w384 h342")
}
...
LaunchPomodoro() {
    ...
    ; Session progress bar — visually fills as the current session counts down
    progBar := pomoGui.Add("Progress", "x10 y156 w340 h14 cGreen Background0xE0E0E0", 0)

    ; Buttons
    btnStart  := pomoGui.Add("Button", "x10  y180 w100 h32", "▶ Start")
    btnPause  := pomoGui.Add("Button", "x116 y180 w100 h32", "⏸ Pause")
    btnReset  := pomoGui.Add("Button", "x222 y180 w100 h32", "↩ Reset")
    btnPause.Enabled := false

    ; Session counter
    pomoGui.SetFont("s9 cGray", "Segoe UI")
    sessionCtrl := pomoGui.Add("Text", "x10 y218 w340 h18 Center", "Sessions completed: 0")
    pomoGui.SetFont("s10 Norm cDefault", "Segoe UI")
    sessionCount := 0
    totalSessionSecs := 0
    ...
    UpdateTimerDisp() {
        ...
        if (totalSessionSecs > 0) {
            elapsed := totalSessionSecs - pomoSecs
            progBar.Value := Round((elapsed / totalSessionSecs) * 100)
        }
    }
