...
The current sequence number is automatically saved to settings.ini after every hotkey press. Progress is never lost, even if the application is closed unexpectedly.
    ...
    mainGui.SetFont("bold s13", "Segoe UI")
    mainGui.Add("Text", "x20 y48 w500 h28 c0x0055AA", "🧰 Mini Apps & Tools")
    mainGui.SetFont("s9 Norm cGray", "Segoe UI")
    mainGui.Add("Text", "x20 y76 w630 h18",
        "Click any tool below to launch it in its own window. Each app runs independently and can be used anytime.")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")

    ; --- ROW 1: Scratchpad | Volume Control ---
    mainGui.Add("GroupBox", "x15 y97 w305 h110", "  📝 Quick Scratchpad")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x28 y118 w280 h42",
        "A floating notepad for quick notes, clipboard drafts, or reminders. Auto-saves content while open.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x28 y160 w180 h18", "Tip: Stays on top of other windows")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchScratch := mainGui.Add("Button", "x195 y155 w115 h28", "📝 Open Notepad")
    btnLaunchScratch.OnEvent("Click", (*) => LaunchScratchpad())

    mainGui.Add("GroupBox", "x330 y97 w305 h110", "  🔊 Volume & Media Control")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x343 y118 w280 h42",
        "Control system volume with a slider and manage media playback — play, pause, skip, and mute.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x343 y160 w175 h18", "Tip: Uses Windows multimedia keys")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchVol := mainGui.Add("Button", "x510 y155 w115 h28", "🔊 Open Volume")
    btnLaunchVol.OnEvent("Click", (*) => LaunchVolumeControl())

    ; --- ROW 2: Screen Dimmer | Cleaner ---
    mainGui.Add("GroupBox", "x15 y217 w305 h110", "  🌙 Screen Dimmer / Eye Care")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x28 y238 w280 h42",
        "Overlay a dark transparent layer on your screen to reduce brightness and eye strain during late-night work.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x28 y280 w180 h18", "Tip: Adjustable opacity slider")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchDimmer := mainGui.Add("Button", "x195 y275 w115 h28", "🌙 Open Dimmer")
    btnLaunchDimmer.OnEvent("Click", (*) => LaunchScreenDimmer())

    mainGui.Add("GroupBox", "x330 y217 w305 h110", "  🗑 Disk Cleaner")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x343 y238 w280 h42",
        "Quickly empty the Recycle Bin and flush the Windows Temp folder to free up disk space in one click.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x343 y280 w180 h18", "Tip: Shows space freed after cleanup")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchCleaner := mainGui.Add("Button", "x510 y275 w115 h28", "🗑 Open Cleaner")
    btnLaunchCleaner.OnEvent("Click", (*) => LaunchDiskCleaner())

    ; --- ROW 3: Live Clock | Pomodoro Timer ---
    mainGui.Add("GroupBox", "x15 y337 w305 h110", "  🕐 Live Clock & Date")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x28 y358 w280 h42",
        "A compact floating clock showing live time and date. Always-on-top so it stays visible over any window.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x28 y400 w180 h18", "Tip: Double-click to toggle date display")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchClock := mainGui.Add("Button", "x195 y395 w115 h28", "🕐 Open Clock")
    btnLaunchClock.OnEvent("Click", (*) => LaunchLiveClock())

    mainGui.Add("GroupBox", "x330 y337 w305 h110", "  🍅 Pomodoro / Work Timer")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x343 y358 w280 h42",
        "Focus timer using the Pomodoro technique: 25-minute work sessions with 5-minute breaks. Beeps on completion.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x343 y400 w180 h18", "Tip: Customizable work/break duration")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchPomo := mainGui.Add("Button", "x510 y395 w115 h28", "🍅 Open Timer")
    btnLaunchPomo.OnEvent("Click", (*) => LaunchPomodoro())

    ; --- ROW 4: Color Picker | Case Converter ---
    mainGui.Add("GroupBox", "x15 y457 w305 h100", "  🎨 Color Picker")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x28 y478 w280 h38",
        "Pick any color on screen with your mouse. Copies the HEX, RGB, and HSL values to clipboard instantly.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x28 y516 w185 h18", "Tip: Press Space to capture pixel color")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchColor := mainGui.Add("Button", "x195 y511 w115 h28", "🎨 Open Picker")
    btnLaunchColor.OnEvent("Click", (*) => LaunchColorPicker())

    mainGui.Add("GroupBox", "x330 y457 w305 h100", "  🔡 Text Case Converter")
    mainGui.SetFont("s9 Norm", "Segoe UI")
    mainGui.Add("Text", "x343 y478 w280 h38",
        "Convert text to UPPER, lower, Title Case, or sentence case. Paste in, click convert, copy out.")
    mainGui.SetFont("s8 cGray Italic", "Segoe UI")
    mainGui.Add("Text", "x343 y516 w185 h18", "Tip: Works on multi-line text too")
    mainGui.SetFont("s10 Norm cDefault", "Segoe UI")
    btnLaunchCase := mainGui.Add("Button", "x510 y511 w115 h28", "🔡 Open Converter")
...
LaunchScratchpad() {
    ...
    scratchGui.OnEvent("Close", (*) => scratchGui.Destroy())

    scratchGui.Add("Text", "x8 y8 w380 h18 cGray", "Your notes are kept while this window is open.")
    scratchEdit := scratchGui.Add("Edit", "x8 y28 w384 h280 +Multi +WantReturn +VScroll +Wrap", scratchContent)

    btnRow := scratchGui.Add("Button", "x8 y316 w90 h28", "📋 Copy All")
    btnRow.OnEvent("Click", (*) => (A_Clipboard := scratchEdit.Value, ToolTip("Copied!"), SetTimer(() => ToolTip(), -1500)))
    btnClear := scratchGui.Add("Button", "x104 y316 w80 h28", "🗑 Clear")
...
LaunchScreenDimmer() {
    ...
    dimmerGui.OnEvent("Close", (*) => (
        (overlayGui != "") ? (overlayGui.Destroy(), overlayGui := "") : "",
        isDimmed := false,
        dimmerGui.Destroy()
    ))

    dimmerGui.SetFont("bold s11 c0x0055AA", "Segoe UI")
    dimmerGui.Add("Text", "x10 y10 w300 h22", "🌙 Screen Dimmer / Eye Care")
    dimmerGui.SetFont("s9 Norm", "Segoe UI")
    dimmerGui.Add("Text", "x10 y36 w310 h30",
        "Applies a dark overlay to reduce screen brightness and eye strain.")

    dimmerGui.Add("Text", "x10 y72 w80 h22 +0x200", "Darkness:")
    dimSlider := dimmerGui.Add("Slider", "x95 y72 w190 h28 +Thick15 Range10-90", 40)
    dimLabel  := dimmerGui.Add("Text", "x290 y72 w40 h22", "40%")
    dimSlider.OnEvent("Change", (*) => (
        dimLabel.Value := dimSlider.Value . "%",
        isDimmed ? UpdateOverlay(dimSlider.Value) : ""
    ))

    btnToggleDim := dimmerGui.Add("Button", "x10 y108 w150 h32", "🌙 Enable Dim")
    btnToggleDim.OnEvent("Click", ToggleDim)

    dimmerGui.Add("Text", "x10 y150 w320 h28 cGray",
        "Tip: Slide to adjust, then click Enable. Press again to turn off.")

    UpdateOverlay(opacity) {
        if (overlayGui == "")
            return
        WinSetTransparent(Round(opacity * 2.55), overlayGui)
    }
    ...
    ToggleDim(*) {
        if (isDimmed) {
            ...
            isDimmed := true
            btnToggleDim.Text := "☀ Disable Dim"
        }
    }

    dimmerGui.Show("w340 h188")
}
...
LaunchLiveClock() {
    ...
    clockGui := Gui("-MaximizeBox +AlwaysOnTop -SysMenu", "🕐 Live Clock")
    ...
    clockGui.SetFont("bold s32 c00d4ff", "Segoe UI")
    timeCtrl := clockGui.Add("Text", "x0 y15 w280 h50 Center", "00:00:00")

    clockGui.SetFont("s11 c88c0d0", "Segoe UI")
    dateCtrl := clockGui.Add("Text", "x0 y65 w280 h22 Center", "")

    clockGui.SetFont("s8 c556677", "Segoe UI")
    clockGui.Add("Text", "x0 y94 w280 h16 Center", "Click to toggle date  |  Close button to exit")

    ; Transparent overlay covers the whole window — handles toggle click
    toggleCtrl := clockGui.Add("Text", "x0 y0 w280 h90 +BackgroundTrans")
    toggleCtrl.OnEvent("Click", ToggleDate)
...
LaunchColorPicker() {
    ...
    cpGui.OnEvent("Close", (*) => cpGui.Destroy())

    cpGui.SetFont("bold s11 c0x0055AA", "Segoe UI")
    cpGui.Add("Text", "x10 y10 w340 h22", "🎨 Screen Color Picker")
    cpGui.SetFont("s9 Norm", "Segoe UI")
    cpGui.Add("Text", "x10 y34 w340 h28",
        "Move your mouse anywhere on screen, then press [Capture] to read the pixel color.")

    ; Color preview box
    cpGui.Add("Text", "x10 y68 w60 h22 +0x200", "Preview:")
    colorBox := cpGui.Add("Progress", "x75 y66 w60 h26 +BackgroundBlack")

    cpGui.Add("Text", "x10 y100 w50 h22 +0x200", "HEX:")
    hexCtrl := cpGui.Add("Edit", "x65 y98 w110 h24 +ReadOnly", "#000000")
    cpGui.Add("Text", "x185 y100 w30 h22 +0x200", "RGB:")
    rgbCtrl := cpGui.Add("Edit", "x220 y98 w120 h24 +ReadOnly", "0, 0, 0")

    cpGui.Add("Text", "x10 y128 w50 h22 +0x200", "Mouse:")
    posCtrl := cpGui.Add("Edit", "x65 y126 w120 h24 +ReadOnly", "X: 0  Y: 0")

    btnCapture := cpGui.Add("Button", "x10 y158 w150 h30", "📷 Capture Color")
    btnCopyHex := cpGui.Add("Button", "x166 y158 w100 h30", "📋 Copy HEX")
    btnCopyRgb := cpGui.Add("Button", "x272 y158 w80 h30", "📋 RGB")

    cpGui.Add("Text", "x10 y196 w340 h18 cGray",
        "Tip: You can also press Space key while hovering over any color.")

    btnCapture.OnEvent("Click", DoCaptureColor)
    ...
    DoCaptureColor(*) {
        ...
        g := (clr >>  8) & 0xFF
        b :=  clr        & 0xFF
        hexVal := Format("#{:02X}{:02X}{:02X}", r, g, b)
        hexCtrl.Value := hexVal
        rgbCtrl.Value := r . ", " . g . ", " . b
        posCtrl.Value := "X: " . mx . "  Y: " . my
        colorBox.Opt("Background" . Format("{:06X}", clr))
    }

    cpGui.Show("w360 h220")
}
