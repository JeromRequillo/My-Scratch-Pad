...
RegisterCustomHotkeys() {
    ...
    for hk in hotkeyList {
        if (hk.key != "" && hk.txt != "") {
            try {
                ; I-convert ang literal \n pabalik sa totoong newlines para sa system hook execution
                ...
                ; Laktawan kung may error sa format ng key para hindi mag-crash
            }
        }
    }
}
...
!F9:: {
    ...
    global current_num
    current_num := Format("{:07}", Number(current_num) + 1)
...
LaunchGUI() {
    ...
    mainGui.SetFont("s9", "Segoe UI")
    mainGui.Add("Text", "x25 y75 w400 h18", "Version: 4.1.0 (Advanced Dynamic Control Panel)")
    ...
    mainGui.Add("Link", "x25 y140 w400 h20", 'Repository: <a href="https://github.com/JeromRequillo/KeyTap-Pro">JeromRequillo/🎯 KeyTap Pro v4.1</a>')
    ...
    CheckConflict(newKey, keyToIgnore := "") {
        global reservedHotkeys, hotkeyList
        for rk in reservedHotkeys {
            if (newKey = rk)
                return "'" . newKey . "' ay reserved ng KeyTap Pro system hotkey!"
        }
        for hk in hotkeyList {
            if (hk.key == keyToIgnore)
                continue
            if (hk.key == newKey)
                return "'" . newKey . "' ay duplicate na ginagamit ng isa pang entry."
        }
        return ""
    }
    ...
    AddUpdateHotkey(*) {
        ...
        ; Hanapin kung nage-exist na sa master array (Update mode)
        existingIdx := 0
        for idx, hk in hotkeyList {
            if (hk.key == newKey) {
                existingIdx := idx
                break
            }
        ...
        conflictMsg := CheckConflict(newKey, (existingIdx > 0) ? newKey : "")
        if (conflictMsg != "") {
            MsgBox("⚠️ Hotkey Conflict Detected!`n`n" . conflictMsg, "Conflict!", 48)
            return
        }
    ...
    DeleteHotkey(*) {
        ...
        targetKey := LV.GetText(selectedRow, 2)
        for idx, hk in hotkeyList {
            if (hk.key == targetKey) {
    ...
    MoveRowUp(*) {
        ...
        if (selectedRow <= 1) {
            if (selectedRow == 0)
                MsgBox("Pumili muna ng row sa listahan.", "Babala", 48)
            return
        }
        
        currentKey := LV.GetText(selectedRow, 2)
        aboveKey := LV.GetText(selectedRow - 1, 2)
        
        idxA := 0, idxB := 0
        for idx, hk in hotkeyList {
            if (hk.key == currentKey)
                idxA := idx
            if (hk.key == aboveKey)
                idxB := idx
        }
        ...
        if (idxA > 0 && idxB > 0) {
            temp := hotkeyList[idxA]
            hotkeyList[idxA] := hotkeyList[idxB]
            hotkeyList[idxB] := temp
        }
        
        RefreshListView(editSearch.Value)
        
        ; I-focus ulit ang ni-reorder na data row
        Loop LV.GetCount() {
            if (LV.GetText(A_Index, 2) == currentKey) {
    ...
    MoveRowDown(*) {
        ...
        currentKey := LV.GetText(selectedRow, 2)
        belowKey := LV.GetText(selectedRow + 1, 2)
    ...
    SaveSettings(*) {
        ...
        current_num := Format("{:07}", Number(guiCtrl_Num.Value)) ; Naka-sync sa 7 digits
        ...
        ; Direct writing mula sa maayos at naka-sequence na master array
        for hk in hotkeyList {
            IniWrite(hk.txt, "settings.ini", "Hotkeys", hk.key)
            IniWrite(hk.enabled ? "1" : "0", "settings.ini", "HotkeyState", hk.key)
        }
